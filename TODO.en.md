...
- [x] Refactor ntopng support into a two-layer design: low-level transport/auth client plus a higher-level normalization/aggregation adapter
...
- [x] Decide to replace the current custom ntopng transport path with the lightweight Python-API-style backend for live queries
...
- [x] Strengthen rollback using native pfSense object identifiers when available instead of description heuristics
- [x] Add real validation in a controlled lab before recommending production use
...
- [ ] Extend host-specific egress blocking beyond the current single-port draft/apply/rollback flow (multiple ports, richer protocols, and unblock by target+port)
